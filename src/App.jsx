...
function FancySheet({sh,totalPages,interactive,currentHp,setCurrentHp,tempHp,setTempHp,deathSaves,setDeathSaves,resourceUses,setResourceUses}){
  ...
  return <div className={`page pro-sheet ornate-sheet theme-${sheetTheme}`}>
    ...
    <div className="panel hp"><div className="panel-titlebar gold">{t("Hit Points")}</div><div className="hp-top"><div><div className="hp-lab">{t("Hit Dice")}</div><div style={{fontSize:"4.2mm",fontWeight:900,marginTop:"0.5mm"}}>{sh.hitDice}</div></div><div><div className="hp-lab">{t("HP Max")}</div><div className="hp-num">{sh.hpMax}</div></div></div>{interactive?<div className="hp-current" style={{color:"#462b10",flexDirection:"column",gap:"1mm"}}><input type="number" value={currentHp??sh.hpMax} onChange={e=>setCurrentHp(Math.max(0,Math.min(sh.hpMax,Number(e.target.value)||0)))} style={{width:"70%",fontSize:"5.5mm",fontWeight:900,textAlign:"center",border:"none",background:"transparent",color:"#462b10"}}/><div style={{display:"flex",alignItems:"center",gap:"1mm",fontSize:"2mm"}}><span style={{color:"#6e4a17",textTransform:"uppercase",fontWeight:700}}>{t("Temporary HP")}</span><input type="number" value={tempHp||0} onChange={e=>setTempHp(Math.max(0,Number(e.target.value)||0))} style={{width:"12mm",fontSize:"2.6mm",fontWeight:900,textAlign:"center",border:"0.35mm solid rgba(107,75,22,.42)",borderRadius:"1mm",background:"rgba(255,255,255,.5)",color:"#462b10"}}/></div></div>:<div className="hp-current">{t("CURRENT HP")}</div>}<div className="death"><div style={{textAlign:"center"}}><div className="subtle-caption" style={{marginBottom:"1.5mm"}}>{t("Successes")}</div><div>{[0,1,2].map(i=>interactive?<span key={i} onClick={()=>setDeathSaves(d=>({...d,success:d.success>i?i:i+1}))} style={{cursor:"pointer",background:(deathSaves?.success||0)>i?"#4ade80":undefined}}/>:<span key={i}/>)}</div></div><div style={{textAlign:"center"}}><div className="subtle-caption" style={{marginBottom:"1.5mm"}}>{t("Failures")}</div><div>{[0,1,2].map(i=>interactive?<span key={i} onClick={()=>setDeathSaves(d=>({...d,fail:d.fail>i?i:i+1}))} style={{cursor:"pointer",background:(deathSaves?.fail||0)>i?"#f87171":undefined}}/>:<span key={i}/>)}</div></div></div></div>
...
export default function App(){
  ...
  },[currentHp,tempHp,deathSaves,resourceUses,usedSlots,racialUses]);
