...
export default function App(){
  ...
  function genSheet(targetView){
    ...
    const subFeatureLines=(sub&&level>=3)?subclassFeaturesAtLevel(cn,sub,level).map(([name,txt])=>{
      ...
      return trSc(name)+" ("+subLabel+"): "+trSc(txt[da?1:0])+psiNote+pickNote+(subPg?" (PHB p."+subPg+")":UA_SUBCLASSES.has(sub)?" (UA 2025)":"");
    }):[];
      ...
      <div style={{marginBottom:"0.85rem",background:"rgba(96,165,250,0.08)",border:"1px solid #60a5fa55",borderRadius:"0.85rem",padding:"0.75rem"}}>
        ...
        <GFld label={level<3?t("Subclass (available at level 3)"):t("Subclass")}>
          <select value={sub} onChange={e=>setSub(e.target.value)} disabled={level<3} style={{...inp,opacity:level<3?0.45:1}}>
            ...
            {Object.keys(SUBCLASSES[cn]||{}).map(s=><option key={s} value={s}>{s}{UA_SUBCLASSES.has(s)?" (UA)":""}</option>)}
          </select>
