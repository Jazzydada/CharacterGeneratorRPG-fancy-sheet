...
export default function App(){
  ...
  function genSheet(){
    ...
    const subFeatureLines=(sub&&level>=3)?subclassFeaturesAtLevel(cn,sub,level).map(([name,txt])=>{const psiNote=(isPsiSub&&name==="Psionic Power")?" ("+psiEnergyDiceCount(level)+psiEnergyDieSize(level)+" Psionic Energy Dice)":"";return trSc(name)+" ("+subLabel+"): "+trSc(txt[da?1:0])+psiNote+(subPg?" (PHB p."+subPg+")":"");}):[];
