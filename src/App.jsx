...
function FancySheet({sh,totalPages,interactive,currentHp,setCurrentHp,tempHp,setTempHp,deathSaves,setDeathSaves,resourceUses,setResourceUses,heroicInspiration,setHeroicInspiration}){
  ...
  return <div className={`page pro-sheet ornate-sheet theme-${sheetTheme}`}>
    ...
    <div className="panel hp"><div className="panel-titlebar gold">{t("Hit Points")}</div>{sh.resistances&&sh.resistances.length>0&&<div style={{position:"relative",display:"flex",flexWrap:"wrap",gap:"1mm",justifyContent:"center",marginTop:"1mm"}}>{sh.resistances.map(r=><span key={r} title={t("Resistance")+": "+trDamageType(r)} style={{fontSize:"1.7mm",fontWeight:700,color:"#7c2d12",border:"0.35mm solid #7c2d12",borderRadius:"3mm",padding:"0.3mm 1.6mm",background:"rgba(255,255,255,.55)",textTransform:"uppercase",letterSpacing:".02em"}}>{trDamageType(r)}</span>)}</div>}<div className="hp-top"><div><div className="hp-lab">{t("Hit Dice")}</div><div style={{fontSize:"4.2mm",fontWeight:900,marginTop:"0.5mm"}}>{sh.hitDice}</div></div><div><div className="hp-lab">{t("HP Max")}</div><div className="hp-num">{sh.hpMax}</div></div></div>{interactive?<div style={{flex:1,display:"flex",flexDirection:"column",gap:"2mm",marginBottom:"2mm",minHeight:0}}>
