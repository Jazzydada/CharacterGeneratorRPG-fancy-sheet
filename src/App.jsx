...
function FancySheet({sh,totalPages,interactive,currentHp,setCurrentHp,tempHp,setTempHp,deathSaves,setDeathSaves,resourceUses,setResourceUses,heroicInspiration,setHeroicInspiration}){
...
function Page3({sh,forms,totalPages}){
  ...
  return(<div className="page" style={{...pgStyle,width:"210mm",height:"297mm",display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    <div style={{flex:"0 0 auto",marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}><span style={{fontSize:8,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.12em",color:GOLD,fontFamily:"sans-serif"}}>{t("Inventory")}</span><span style={{fontSize:8,fontFamily:"serif"}}><span style={{fontWeight:700}}>{(()=>{const c=sh.coins||{};const parts=[["pp","PP"],["gp","GP"],["ep","EP"],["sp","SP"],["cp","CP"]].filter(([k])=>c[k]).map(([k,l])=>c[k]+" "+l);return parts.length?parts.join(" "):"0 GP";})()}</span><span style={{color:GOLD,fontWeight:700,marginLeft:6}}>= {(coinsTotalCP(sh.coins||{})/100).toFixed(2)} GP</span></span></div>
