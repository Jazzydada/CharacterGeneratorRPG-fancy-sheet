...
function SpellLevelCards({sh,spellsByLevel,interactive,spPrep,setSpPrep}){
  const{spellSlots}=sh;
  return <>{LVLL.map((lvl,li)=>{const spells=spellsByLevel[li]||[];if(!spells.length)return null;return <div key={lvl} style={{marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><div style={{fontSize:9.5,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.12em",color:GOLD,fontFamily:"sans-serif",whiteSpace:"nowrap"}}>{lvl}</div>{li>0&&<div style={{...capL,fontSize:9.5,marginBottom:0}}>{spellSlots[li-1]||0} slots</div>}<div style={{flex:1,height:"0.5px",background:RULE}}/></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:5}}>{spells.map((sp,i)=>{
...
function FancySheet({sh,totalPages,interactive,currentHp,setCurrentHp,tempHp,setTempHp,deathSaves,setDeathSaves,resourceUses,setResourceUses,heroicInspiration,setHeroicInspiration,coins,setCoins}){
...
export default function App(){
  ...
  },[currentHp,tempHp,deathSaves,resourceUses,usedSlots,racialUses,heroicInspiration,exhaustion,spPrep,coins,inventory,backstory]);
