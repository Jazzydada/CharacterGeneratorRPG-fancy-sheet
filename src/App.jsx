...
const SPELL_SPLIT_THRESHOLD=26;
...
// Splits at an exact spell-level boundary (never mid-level) — used once the real DOM measurement
// (see spellSectionRef below) has determined which level is the first one that doesn't fit on page 2.
function splitSpellsByLevelAt(spellsByLevel,cutLevel){
  const page1={},page2={};
  for(let li=0;li<10;li++){
    const arr=spellsByLevel[li]||[];
    if(cutLevel===null||li<cutLevel){page1[li]=arr;page2[li]=[];}
    else{page1[li]=[];page2[li]=arr;}
  }
  return[page1,page2];
}
...
// When either continuation page above exists, the Backstory box no longer tries to share space with
// dense spell/feature content (that's what was squashing it against the footer) — it gets a full page
// of its own instead, inserted before the original Inventory page (which keeps its own content, just
// renumbered).
function BackstoryPage({sh,pageNum,totalPages,interactive,backstory,setBackstory}){
  return(<div className="page" style={{...pgStyle,width:"210mm",height:"297mm",display:"flex",flexDirection:"column",overflow:"hidden"}}>
    <div style={{flex:"0 0 auto",display:"flex",justifyContent:"space-between",alignItems:"flex-end",borderBottom:"1.5px solid "+GOLD_L,paddingBottom:5,marginBottom:6}}>
      <div><div style={{fontSize:16,fontWeight:700,fontFamily:"serif"}}>{sh.name}</div><div style={{...capL,fontSize:9.5}}>{classLevelSubOf(sh)} - {t("Backstory")}</div></div>
    </div>
    <div style={{flex:"1 1 0",minHeight:0,display:"flex",flexDirection:"column",marginTop:2}}>
      ...
      <div style={{flex:1,minHeight:0,overflow:"hidden",border:"1px solid "+RULE,borderRadius:4,padding:"6px 8px",background:"#fff"}}>
        {interactive&&setBackstory?<textarea value={backstory??sh.backstory??""} onChange={e=>setBackstory(e.target.value)} style={{fontSize:9.5,lineHeight:1.5,fontFamily:"sans-serif",color:"#222",whiteSpace:"pre-wrap",width:"100%",height:"100%",minHeight:0,minWidth:0,border:"none",outline:"none",resize:"none",overflow:"auto",boxSizing:"border-box",background:"transparent"}}/>:<>
        <div style={{fontSize:9.5,lineHeight:1.5,fontFamily:"sans-serif",color:"#222",whiteSpace:"pre-wrap"}}>{sh.backstory||""}</div>
        {!sh.backstory&&<div>{Array.from({length:12}).map((_,i)=><div key={i} style={{borderBottom:"0.5px dashed #ddd",height:"5.5mm"}}/>)}</div>}
        </>}
      </div>
    </div>
    <div style={{flex:"0 0 auto",marginTop:5,borderTop:"0.5px solid "+RULE,paddingTop:3,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:9.5,color:GOLD,fontFamily:"sans-serif"}}>D&D 2024 SRD 5.2</span><span style={{fontSize:9.5,color:GOLD,fontFamily:"sans-serif"}}>Page {pageNum} of {totalPages}</span></div>
...
function FancySheet({sh,totalPages,interactive,currentHp,setCurrentHp,tempHp,setTempHp,deathSaves,setDeathSaves,resourceUses,setResourceUses,heroicInspiration,setHeroicInspiration,coins,setCoins,hitDiceUsed,setHitDiceUsed}){
  ...
  return <div className={`page pro-sheet ornate-sheet theme-${sheetTheme}`}>
    ...
    <div className="ribbon insp-label">{t("Inspiration")}</div><div className="small-token inspiration" onClick={interactive?()=>setHeroicInspiration(v=>!v):undefined} style={interactive?{cursor:"pointer",background:heroicInspiration?"radial-gradient(circle,#fff3c4 0 48%,#e8b93a 88%)":undefined}:undefined}><b>{interactive?(heroicInspiration?"✦":"—"):0}</b></div>
    ...
    <div className="panel hp"><div className="panel-titlebar gold">{t("Hit Points")}</div>{sh.resistances&&sh.resistances.length>0&&<div style={{flex:"0 0 auto",display:"flex",flexWrap:"wrap",justifyContent:"flex-end",gap:"0.7mm",marginBottom:"1.5mm"}}>{sh.resistances.map(r=><span key={r} title={t("Resistance")+": "+trDamageType(r)} style={{fontSize:"2.5mm",fontWeight:700,color:"#fde3d3",background:"#7c2d12",borderRadius:"2.7mm",padding:"0.5mm 1.6mm",display:"flex",alignItems:"center",gap:"0.6mm",textTransform:"uppercase",letterSpacing:".02em",whiteSpace:"nowrap"}}><span>{DAMAGE_EMOJI[r]||"●"}</span>{trDamageType(r)}</span>)}</div>}<div className="hp-top"><div><div className="hp-lab">{t("Hit Dice")}</div><div style={{fontSize:"4.2mm",fontWeight:900,marginTop:"0.5mm"}}>{sh.hitDice}</div>{interactive&&setHitDiceUsed&&(()=>{const total=Number.parseInt((sh.hitDice||"").split("d")[0],10)||0;return total>0&&<div style={{display:"flex",flexWrap:"wrap",gap:"0.6mm",marginTop:"0.8mm"}}>{Array.from({length:Math.min(total,20)}).map((_,j)=>{const used=(hitDiceUsed||0)>j;return <span key={j} onClick={()=>setHitDiceUsed(prev=>used?j:j+1)} title={t("Hit Die")} style={{width:"2.6mm",height:"2.6mm",borderRadius:"50%",border:".4mm solid #7b5118",background:used?"#7b5118":"#fff4d3",display:"inline-block",cursor:"pointer"}}/>;})}</div>;})()}</div><div><div className="hp-lab">{t("HP Max")}</div><div className="hp-num">{sh.hpMax}</div></div></div>{interactive?<div style={{flex:1,display:"flex",flexDirection:"column",gap:"2mm",marginBottom:"2mm",minHeight:0}}>
...
function Page1({sh}){
  ...
  return(<div className="page" style={pgStyle}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:2}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        <div style={{background:"#fff",border:"1px solid "+RULE,borderRadius:4,padding:"5px 7px"}}><div style={{display:"flex",justifyContent:"center",alignItems:"baseline",gap:5,borderBottom:"0.5px solid "+RULE,marginBottom:4,paddingBottom:2}}><span style={{fontSize:7,textTransform:"uppercase",letterSpacing:"0.14em",fontWeight:700,color:GOLD,fontFamily:"sans-serif"}}>Currency</span>{(()=>{const c=displayCoins;const mixed=[c.cp,c.sp,c.ep,c.gp,c.pp].filter(Boolean).length>1;if(!mixed)return null;const totalGp=coinsTotalCP(c)/100;const totalStr=Number.isInteger(totalGp)?String(totalGp):totalGp.toFixed(2);return<span style={{fontSize:6.5,fontWeight:700,fontFamily:"serif",color:INK}}>({totalStr} GP)</span>;})()}</div><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:3,textAlign:"center"}}>{[["cp","CP","#b87333"],["sp","SP","#aaa"],["ep","EP","#8fbc8f"],["gp","GP","#d4af37"],["pp","PP","#e5e4e2"]].map(([k,l,c])=><div key={l} style={{textAlign:"center"}}><CoinCircle value={displayCoins[k]||0} interactive={interactive&&!!setCoins} onChange={v=>setCoins(prev=>({...prev,[k]:v}))} size={28} fontSize={7} bg={c}/><div style={{...capL,textAlign:"center",marginTop:4,fontSize:6}}>{l}</div></div>)}</div></div>
...
function Page2({sh,totalPages,interactive,usedSlots,setUsedSlots,racialUses,setRacialUses,spPrep,setSpPrep,spellsByLevelOverride,featLinesOverride,hideBackstory,backstory,setBackstory,spellSectionRef}){
  ...
  return(<div className="page" style={{...pgStyle,width:"210mm",height:"297mm",display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    {isCaster&&<div ref={spellSectionRef} style={{flex:"0 1 auto",overflow:"hidden"}}>
...
function SPopup({name,d,onClose}){
  return(<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#0f172a",border:"1px solid #fcd34d",borderRadius:"1.25rem",padding:"1.5rem",maxWidth:"420px",width:"100%"}}>
      ...
      {d.pg&&<div style={{fontSize:"0.62rem",color:G.dim,marginTop:"0.6rem"}}>{typeof d.pg==="string"?pgLabel(d.pg):"PHB 2024, p. "+d.pg}</div>}
    </div>
  </div>);
}
  ...
  return(<div style={{background:sel?"#14532d22":"transparent",border:"1px solid "+(sel?"#4ade8066":G.border),borderRadius:"0.6rem",padding:"0.4rem 0.6rem"}}>
    <div style={{display:"flex",alignItems:"flex-start",gap:"0.5rem"}}>
      ...
      <div style={{flex:1}}>
        ...
        {feat.pg&&<div style={{fontSize:"0.62rem",color:G.dimmer,marginTop:"2px"}}>{pgLabel(feat.pg)}</div>}
      </div>
    </div>
    {children}
  </div>);
...
export default function App(){
  ...
  async function downloadImagePdf(){
    ...
    // The pinch-zoom-pan view puts a translate+scale transform on the pages' shared ancestor, which
    // can throw off html-to-image's capture of the individual .page nodes (wrong size/position in the
    // exported image). Reset it to a plain 1:1 layout for the duration of the export, then restore it.
    const innerEl=document.querySelector(".sheet-fit-inner");
    ...
    try{
      ...
      alert(t("Could not create image PDF")+": "+e.message);
    }finally{
      if(innerEl)innerEl.style.transform=prevTransform;
  ...
  const [selManeuvers,setSelManeuvers]=useState([]);
  const [selHunterPrey,setSelHunterPrey]=useState("");
  const [selDefensiveTactics,setSelDefensiveTactics]=useState("");
  const [selBeastType,setSelBeastType]=useState("");
  const [vestigeDomain,setVestigeDomain]=useState("Life Domain");
  ...
  // Real DOM measurement (not a guess) decides where the spell list has to break onto a continuation
  // page: whatever level's card first extends past the spell section's actual available bottom edge
  // is the cut point. See the useLayoutEffect below, next to the sheet-view render block.
  ...
  useLayoutEffect(()=>{
    if(!sheet)return;
    if(lastMeasuredSheetRef.current!==sheet){
      // A new sheet: force a full, unsplit render first (so the measurement below reflects real
      // content, not a stale split from the previous character) before measuring on the next pass.
      lastMeasuredSheetRef.current=sheet;
      if(spellSplitLevel!==null){setSpellSplitLevel(null);return;}
      // Already null (already showing full content this render) — fall through and measure now
      // instead of bailing out, otherwise a first-ever overflow with no prior split is missed
      // entirely (setSpellSplitLevel(null) on an already-null value is a no-op, so nothing would
      // ever trigger the measurement below).
    }
    const el=spellSectionRef.current;
    if(!el)return;
    if(el.scrollHeight<=el.clientHeight+1)return;
    const bottom=el.getBoundingClientRect().bottom;
    const groups=[...el.querySelectorAll("[data-lvl]")];
    let cut=null;
    for(const g of groups){
      if(g.getBoundingClientRect().bottom>bottom){cut=Number(g.getAttribute("data-lvl"));break;}
    }
  ...
  // Keeps the pan/zoom view from being dragged so far that the sheet leaves the viewport entirely —
  // clamps x/y so the content's edges can't pass the container's opposite edge, like a photo viewer.
  const clampSheetView=(pos)=>{
    const el=sheetContainerRef.current;
    const inner=el?.firstElementChild;
    if(!el||!inner)return pos;
    const cw=el.clientWidth,ch=el.clientHeight;
    const iw=inner.offsetWidth*pos.scale,ih=inner.offsetHeight*pos.scale;
    const minX=Math.min(0,cw-iw),maxX=Math.max(0,cw-iw);
    const minY=Math.min(0,ch-ih),maxY=Math.max(0,ch-ih);
    return{scale:pos.scale,x:Math.min(maxX,Math.max(minX,pos.x)),y:Math.min(maxY,Math.max(minY,pos.y))};
  };
  ...
  useEffect(()=>{
    const el=sheetContainerRef.current;
    if(!el||view!=="sheet"||!interactiveMode)return;
    ...
    function onTouchMove(e){
      ...
      if(g.mode==="pinch"&&e.touches.length===2){
        ...
        sheetViewRef.current=clampSheetView({scale:newScale,x:m.x-contentX*newScale,y:m.y-contentY*newScale});
        bumpSheetRender();
        ...
        const cur=sheetViewRef.current;
        sheetViewRef.current=clampSheetView({...cur,x:cur.x+dx,y:cur.y+dy});
        bumpSheetRender();
      }
    }
    ...
    function onWheel(e){
      ...
      if(e.ctrlKey||e.metaKey){
        ...
        sheetViewRef.current=clampSheetView({...cur,x:cur.x-e.deltaX,y:cur.y-e.deltaY});
      }
      bumpSheetRender();
    }
  ...
  const isBattleMaster=cn==="Fighter"&&sub==="Battle Master";
  const maneuverLimit=isBattleMaster?maneuversKnown(lv1e):0;
  function togManeuver(name){setSelManeuvers(prev=>{if(prev.includes(name))return prev.filter(n=>n!==name);if(prev.length>=maneuverLimit)return prev;return[...prev,name];});}
  const isHunter=cn==="Ranger"&&sub==="Hunter";
  const isBeastMaster=cn==="Ranger"&&sub==="Beast Master";
  ...
  const avSp=useMemo(()=>{
    ...
    [cn,...(mc&&CTYPE[cn2]?[cn2]:[])].forEach(c=>{
      ...
      const sd=CS[cIsThird?thirdCasterList(c):c]||{};const cct=CTYPE[c]||(cIsThird?"third":undefined);const cMaxSL=cct?maxSpellLevel(cct,c===cn?lv1e:lv2c):0;
  ...
  const thirdCantrips=thirdLvl>=3?(cn==="Fighter"||cn==="Monk"?(thirdLvl>=10?3:2):(thirdLvl>=10?4:3)):0;
  ...
  },[currentHp,tempHp,deathSaves,resourceUses,usedSlots,racialUses,heroicInspiration,exhaustion,hitDiceUsed,spPrep,coins,inventory,backstory]);
  ...
  function buildSBL(){
    ...
    if(sub&&level>=3){
      ...
      const isVestige=cn==="Warlock"&&sub==="Vestige Patron";
      const bonusNames=isCircleLand?circleLandSpellsAtLevel(landType,level):isVestige?subclassSpellsAtLevel("Cleric",vestigeDomain,level):subclassSpellsAtLevel(cn,sub,level);
      const bonusLabel=isCircleLand?sub+" ("+landType+")":isVestige?sub+" ("+vestigeDomain+")":sub;
    ...
    if(cn==="Warlock"&&!mc){
      ...
      Object.keys(res).forEach(k=>{
        ...
        if(li>0&&li!==pactLvl){
          ...
          res[li].forEach(s=>{
            const upDesc=upcastText(s.name,pactLvl);
            res[pactLvl].push(upDesc&&upDesc!==s.desc?{...s,desc:upDesc}:s);
          });
          delete res[li];
        }
      });
  ...
  function genSheet(targetView){
    ...
    const featPgTxt=n=>ALL_FEATS[n]?.pg?" ("+pgLabel(ALL_FEATS[n].pg)+")":"";
    ...
    const isPsiSub=(sub==="Psi Warrior"||sub==="Soulknife");
    const subFeatureLines=(sub&&level>=3)?subclassFeaturesAtLevel(cn,sub,level).map(([name,txt])=>{
      const psiNote=(isPsiSub&&name==="Psionic Power")?" ("+psiEnergyDiceCount(level)+psiEnergyDieSize(level)+" Psionic Energy Dice)":"";
      ...
      return trSc(name)+" ("+subLabel+"): "+trSc(txt[da?1:0])+psiNote+pickNote+(subPg?" ("+pgLabel(subPg)+")":"");
    }):[];
    const classFeaturesTxt=[...(cls.features||[]).filter(f=>!(sub&&/^Subclass\b/i.test(f))).filter(f=>featureLevel(f)<=level).map(f=>{const label=da?(FEATURE_DA[f]||f):f;const d=(cn==="Cleric"&&f==="Blessed Strikes Lvl7")?blessedStrikesText(blessedStrikes,level)[da?1:0]+" ("+t(blessedStrikes)+")":FEATURE_DESC[f]?.[da?1:0];const cfPg=CLASS_FEATURE_PG[cn]?.[f];const pgSuffix=cfPg&&!/PHB p\./.test(d||"")?" (PHB p."+cfPg+")":"";return(d?label+": "+d:label)+pgSuffix;}),...subFeatureLines].filter(Boolean).join("\n");
    const breathDC=8+cm+pb;
    const dragonTraitDetail={
      "Draconic Ancestry":dragonColor+" — "+trDamageType(DRACONIC_ANCESTRY[dragonColor]),
      "Breath Weapon":breathWeaponDice(level)+" "+trDamageType(DRACONIC_ANCESTRY[dragonColor])+", DC "+breathDC+" DEX save (half on success), 15-ft Cone or 30x5-ft Line, "+pb+"/long rest",
      "Damage Resistance":da?("Resistance mod "+trDamageType(DRACONIC_ANCESTRY[dragonColor])+"-skade"):("Resistance to "+DRACONIC_ANCESTRY[dragonColor]+" damage"),
    };
    ...
    if(sp==="Dragonborn")nextResistances.push(DRACONIC_ANCESTRY[dragonColor]);
    ...
    const maneuverLine=(isBattleMaster&&selManeuvers.length)?selManeuvers.map(n=>{const d=MANEUVER_OPTIONS[n];return "• "+n+": "+d[da?1:0]+" (PHB p."+d[2]+")";}).join("\n"):"";
    const maneuverBlockTxt=maneuverLine?"Maneuvers ("+superiorityDice(lv1e)+superiorityDieSize(lv1e)+" Superiority Dice):\n"+maneuverLine:"";
    ...
    const combinedFeatures=[orderLine,featsList,invBlock,metamagicBlockTxt,maneuverBlockTxt,wildShapeLine,rageLine,channelDivinityLine,sorceryPointsLine,unarmoredMoveLine,classFeaturesTxt,racialTraitsTxt].filter(Boolean).join("\n\n--\n\n");
    ...
    // Action Surge (PHB 2024 p.129): a second Fighter class resource alongside Second Wind — tracked
    // separately here since classResource() only returns one primary resource per class.
    const fighterLevelForAS=cn==="Fighter"?lv1e:(mc&&cn2==="Fighter"?lv2c:0);
    const nextResourceAS=fighterLevelForAS>=2?{name:"Action Surge",uses:fighterActionSurgeUses(fighterLevelForAS),recharge:"all/Short or Long Rest",desc:RESOURCE_DESC["Action Surge"]}:null;
  ...
  const longRest=()=>{
    setCurrentHp(sheet.hpMax);
    setTempHp(0);
    setDeathSaves({success:0,fail:0});
    setResourceUses({});
    setRacialUses({});
    setUsedSlots({});
    setExhaustion(e=>Math.max(0,e-1));
    setHitDiceUsed(0);
  };
  ...
  if(view==="sheet"&&sheet){
    ...
    // half across a page break reads badly). Whether Spells needs to spill onto a continuation page,
    // and exactly which level to cut at, comes from real DOM measurement (spellSectionRef + the
    // useLayoutEffect above) rather than a guessed line/spell count — that guess was consistently
    // wrong (leaving half a page empty on both sides of the cut for feature-heavy casters).
    const allFeatLines=parseFeatureLines(sheet.features);
    const needsSpellOverflow=sheet.isCaster&&spellSplitLevel!==null;
    ...
    if(!interactiveMode){
      ...
      // panned or transformed during capture, so browser printing and the PDF export both work exactly
      // as they render here. Pinch-zoom-pan is reserved for "Interaktivt ark" below.
      const fitScale=sheetFitScale();
      return <div><div className="no-print" style={{display:"flex",gap:8,padding:"8px 14px",background:"#1a0e00",alignItems:"center",flexWrap:"wrap"}}><button onClick={()=>setView("gen")} style={{padding:"5px 14px",borderRadius:4,border:"1px solid #c9a84c",background:"#2d1a00",color:"#fcd34d",cursor:"pointer",fontSize:12,fontWeight:600}}>{t("Back")}</button><button onClick={downloadImagePdf} disabled={exportingImg} style={{padding:"5px 14px",borderRadius:4,border:"1px solid #4ade80",background:"#14532d",color:"#4ade80",cursor:exportingImg?"wait":"pointer",fontSize:12,fontWeight:600,opacity:exportingImg?0.6:1}}>{exportingImg?t("Generating…"):t("Download PDF")}</button><span style={{fontSize:11,color:"#8a6a2a"}}>{totalPages+" "+t("pages")}</span></div><div className="sheet-fit-outer" style={{width:PAGE_W_PX*fitScale,height:PAGE_H_PX*totalPages*fitScale,overflow:"hidden",margin:"0 auto"}}><div className="print-area sheet-fit-inner" style={{transform:`scale(${fitScale})`,transformOrigin:"top left"}}>{pagesJsx}</div></div>{sharedStyle}</div>;
    }
    const fitScale=sheetFitScale();
    const view_=sheetViewRef.current;
    const zoomPct=Math.round((view_.scale/fitScale)*100);
    return <div style={{display:"flex",flexDirection:"column",height:"100dvh"}}><div className="no-print" style={{display:"flex",gap:8,padding:"8px 14px",background:"#1a0e00",alignItems:"center",flexWrap:"wrap",flex:"0 0 auto"}}><button onClick={()=>setView("gen")} style={{padding:"5px 14px",borderRadius:4,border:"1px solid #c9a84c",background:"#2d1a00",color:"#fcd34d",cursor:"pointer",fontSize:12,fontWeight:600}}>{t("Back")}</button><button onClick={downloadImagePdf} disabled={exportingImg} style={{padding:"5px 14px",borderRadius:4,border:"1px solid #4ade80",background:"#14532d",color:"#4ade80",cursor:exportingImg?"wait":"pointer",fontSize:12,fontWeight:600,opacity:exportingImg?0.6:1}}>{exportingImg?t("Generating…"):t("Download PDF")}</button><span style={{fontSize:11,color:"#8a6a2a"}}>{totalPages+" "+t("pages")}</span><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:11,color:"#8a6a2a"}}>{t("Exhaustion")}:</span><div style={{display:"flex",gap:2}}>{Array.from({length:6}).map((_,i)=>{const filled=exhaustion>i;return <span key={i} onClick={()=>setExhaustion(exhaustion>i?i:i+1)} title={String(i+1)} style={{width:12,height:12,borderRadius:"50%",border:"1px solid #c9a84c",cursor:"pointer",background:filled?"#c9a84c":"transparent",display:"inline-block"}}/>;})}</div><button onClick={longRest} title={t("Long Rest")} style={{padding:"5px 10px",borderRadius:4,border:"1px solid #60a5fa",background:"#1e3a5f",color:"#93c5fd",cursor:"pointer",fontSize:12,fontWeight:600}}>🌙 {t("Long Rest")}</button></div><div style={{display:"flex",alignItems:"center",gap:4,marginLeft:"auto"}}><button onClick={()=>{sheetViewRef.current=clampSheetView({...view_,scale:Math.max(0.35,+(view_.scale-fitScale*0.2).toFixed(3))});bumpSheetRender();}} title={t("Zoom out")} style={{width:26,height:26,borderRadius:4,border:"1px solid #c9a84c",background:"#2d1a00",color:"#fcd34d",cursor:"pointer",fontSize:14,fontWeight:700}}>−</button><span style={{fontSize:11,color:"#8a6a2a",minWidth:34,textAlign:"center"}}>{zoomPct}%</span><button onClick={()=>{sheetViewRef.current=clampSheetView({...view_,scale:Math.min(3.5,+(view_.scale+fitScale*0.2).toFixed(3))});bumpSheetRender();}} title={t("Zoom in")} style={{width:26,height:26,borderRadius:4,border:"1px solid #c9a84c",background:"#2d1a00",color:"#fcd34d",cursor:"pointer",fontSize:14,fontWeight:700}}>+</button>{zoomPct!==100&&<button onClick={resetSheetView} style={{padding:"4px 8px",borderRadius:4,border:"1px solid #c9a84c",background:"#2d1a00",color:"#fcd34d",cursor:"pointer",fontSize:11}}>{t("Reset")}</button>}</div></div><div ref={sheetContainerRef} className="sheet-fit-outer" style={{width:"100%",flex:"1 1 auto",minHeight:0,overflow:"hidden",position:"relative",background:"#0a0a0a",touchAction:"none",cursor:"grab"}}><div className="print-area sheet-fit-inner" style={{transform:`translate(${view_.x}px,${view_.y}px) scale(${view_.scale})`,transformOrigin:"top left",willChange:"transform"}}>{pagesJsx}</div></div>{sharedStyle}</div>;
  }
  ...
  const buildOverview=()=>{
    ...
    const weaponDisplay=[...buildW(),...(sp==="Dragonborn"?[{name:"Breath Weapon ("+dragonColor+")",atk:"DC "+(8+cm+pb),dmg:breathWeaponDice(level)+" "+trDamageType(DRACONIC_ANCESTRY[dragonColor]),props:"15-ft Cone or 30x5-ft Line",mastery:"—"}]:[])];
    return(<div>
  ...
  const identityBasicsPanel=(
    <div>
      ...
      <GFld label={t("Portrait")}><div style={{display:"flex",gap:"0.5rem"}}>{[["upload",t("Upload Image")],["blank",t("Draw your own")]].map(([m,lbl])=><button key={m} onClick={()=>setPortraitMode(m)} style={{...tabSt(portraitMode===m),flex:1}}>{lbl}</button>)}</div></GFld>
    ...
    <div>
      <div style={{marginBottom:"0.85rem",background:"rgba(96,165,250,0.08)",border:"1px solid #60a5fa55",borderRadius:"0.85rem",padding:"0.75rem"}}>
        ...
        <GFld label={t("Class")}><select value={cn} onChange={e=>{changeClass(e.target.value);classLockedRef.current=true;setClassLocked(true);}} style={inp}>{Object.keys(CLASSES).map(c=><option key={c}>{c}</option>)}</select>{cls&&<div style={{marginTop:"0.4rem",background:G.card,borderRadius:"0.65rem",padding:"0.5rem 0.65rem"}}><div style={{fontSize:"0.65rem",color:"#60a5fa",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:"0.3rem",fontWeight:700}}>{t("Class Features")}</div>{cls.features.filter(f=>{const m=f.match(/Lvl(\d+)/);return(m?parseInt(m[1],10):1)<=level;}).map((f,i)=>{const da=CURRENT_LANG==="da";const label=da?(FEATURE_DA[f]||f):f;const d=FEATURE_DESC[f]?.[da?1:0];return <div key={i} style={{fontSize:"0.73rem",color:G.muted,marginBottom:"0.25rem"}}>- <b style={{color:"#cbd5e1"}}>{label}</b>{d?<span style={{color:G.dim}}> — {d}</span>:""}</div>;})}</div>}
          ...
          {cn==="Cleric"&&level>=7&&<div style={{marginTop:"0.6rem",background:"#2d1a00",border:"1px solid "+G.gold,borderRadius:"0.75rem",padding:"0.6rem 0.7rem"}}><div style={{fontSize:"0.78rem",color:G.gold,marginBottom:"0.4rem",fontWeight:800,display:"flex",alignItems:"center",gap:"0.4rem"}}>⚡ {t("Choose")}: {t("Blessed Strikes")}</div><div style={{display:"flex",flexDirection:"column",gap:"0.35rem"}}>{Object.keys(BLESSED_STRIKES_OPTIONS).map(nm=>{const sel=blessedStrikes===nm;const desc=blessedStrikesText(nm,level);return <button key={nm} onClick={()=>setBlessedStrikes(nm)} style={{textAlign:"left",padding:"0.45rem 0.6rem",borderRadius:"0.6rem",border:"1px solid "+(sel?G.gold:"#334155"),background:sel?"#4a3800":"#0f172a",cursor:"pointer"}}><div style={{fontSize:"0.8rem",fontWeight:700,color:sel?G.gold:"#e2e8f0"}}>{sel?"✓ ":""}{t(nm)}</div><div style={{fontSize:"0.7rem",color:G.muted,marginTop:"1px"}}>{desc[CURRENT_LANG==="da"?1:0]}</div></button>;})}</div></div>}</GFld>
        ...
        {isBattleMaster&&<div style={{marginTop:"0.6rem",background:"#1a1035",border:"1px solid #4c1d95",borderRadius:"0.85rem",padding:"0.65rem 0.7rem"}}>
          <div style={{display:"flex",alignItems:"center",gap:"0.5rem",flexWrap:"wrap",marginBottom:"0.5rem"}}>
            <span style={{fontSize:"0.78rem",fontWeight:800,color:"#c4b5fd"}}>{t("Maneuvers")}</span>
            <span style={{fontSize:"0.7rem",fontWeight:700,color:selManeuvers.length>=maneuverLimit?"#4ade80":"#c4b5fd",background:"#2e1065",border:"1px solid #6d28d9",borderRadius:"0.5rem",padding:"0.1rem 0.45rem"}}>{selManeuvers.length} / {maneuverLimit}</span>
            <span style={{fontSize:"0.65rem",color:G.dim}}>{superiorityDice(lv1e)}{superiorityDieSize(lv1e)} {t("Superiority Dice")}</span>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:"0.3rem",maxHeight:"40vh",overflowY:"auto",paddingRight:"0.25rem"}}>
            {Object.entries(MANEUVER_OPTIONS).map(([name,info])=>{const sel=selManeuvers.includes(name);const atMax=selManeuvers.length>=maneuverLimit;const blocked=!sel&&atMax;return(
              <div key={name} onClick={()=>!blocked&&togManeuver(name)} style={{display:"flex",alignItems:"flex-start",gap:"0.5rem",padding:"0.35rem 0.55rem",borderRadius:"0.55rem",cursor:blocked?"not-allowed":"pointer",opacity:blocked?0.4:1,background:sel?"#4c1d9544":"transparent",border:"1px solid "+(sel?"#a78bfa":"#332255")}}>
                <span style={{flexShrink:0,width:"1rem",height:"1rem",borderRadius:"0.3rem",border:"1px solid "+(sel?"#a78bfa":"#555"),background:sel?"#7c3aed":"transparent",color:"#fff",fontSize:"0.7rem",fontWeight:900,textAlign:"center",lineHeight:"0.95rem",marginTop:"1px"}}>{sel?"✓":""}</span>
                <div style={{flex:1}}><div style={{fontSize:"0.75rem",fontWeight:700,color:sel?"#e9d5ff":"#e2e8f0"}}>{name}</div><div style={{fontSize:"0.68rem",color:G.muted,lineHeight:1.3}}>{info[CURRENT_LANG==="da"?1:0]}</div></div>
              </div>
            );})}
          </div>
        </div>}
        {isHunter&&<div style={{marginTop:"0.6rem",background:"#1a1035",border:"1px solid #4c1d95",borderRadius:"0.85rem",padding:"0.65rem 0.7rem"}}>
          <div style={{fontSize:"0.75rem",fontWeight:800,color:"#c4b5fd",marginBottom:"0.35rem"}}>{t("Hunter's Prey")}</div>
          <div style={{display:"flex",flexDirection:"column",gap:"0.3rem",marginBottom:level>=7?"0.6rem":0}}>
            {Object.entries(HUNTER_PREY_OPTIONS).map(([name,info])=>{const sel=selHunterPrey===name;return(
              <div key={name} onClick={()=>setSelHunterPrey(name)} style={{display:"flex",alignItems:"flex-start",gap:"0.5rem",padding:"0.35rem 0.55rem",borderRadius:"0.55rem",cursor:"pointer",background:sel?"#4c1d9544":"transparent",border:"1px solid "+(sel?"#a78bfa":"#332255")}}>
                <span style={{flexShrink:0,width:"1rem",height:"1rem",borderRadius:"50%",border:"1px solid "+(sel?"#a78bfa":"#555"),background:sel?"#7c3aed":"transparent",marginTop:"1px"}}/>
                <div style={{flex:1}}><div style={{fontSize:"0.75rem",fontWeight:700,color:sel?"#e9d5ff":"#e2e8f0"}}>{name}</div><div style={{fontSize:"0.68rem",color:G.muted,lineHeight:1.3}}>{info[CURRENT_LANG==="da"?1:0]}</div></div>
              </div>
            );})}
          </div>
          {level>=7&&<><div style={{fontSize:"0.75rem",fontWeight:800,color:"#c4b5fd",marginBottom:"0.35rem"}}>{t("Defensive Tactics")}</div>
          <div style={{display:"flex",flexDirection:"column",gap:"0.3rem"}}>
            {Object.entries(DEFENSIVE_TACTICS_OPTIONS).map(([name,info])=>{const sel=selDefensiveTactics===name;return(
              <div key={name} onClick={()=>setSelDefensiveTactics(name)} style={{display:"flex",alignItems:"flex-start",gap:"0.5rem",padding:"0.35rem 0.55rem",borderRadius:"0.55rem",cursor:"pointer",background:sel?"#4c1d9544":"transparent",border:"1px solid "+(sel?"#a78bfa":"#332255")}}>
                <span style={{flexShrink:0,width:"1rem",height:"1rem",borderRadius:"50%",border:"1px solid "+(sel?"#a78bfa":"#555"),background:sel?"#7c3aed":"transparent",marginTop:"1px"}}/>
                <div style={{flex:1}}><div style={{fontSize:"0.75rem",fontWeight:700,color:sel?"#e9d5ff":"#e2e8f0"}}>{name}</div><div style={{fontSize:"0.68rem",color:G.muted,lineHeight:1.3}}>{info[CURRENT_LANG==="da"?1:0]}</div></div>
              </div>
            );})}
        ...
        {cn==="Warlock"&&sub==="Vestige Patron"&&<div style={{marginTop:"0.6rem",background:"#1a1035",border:"1px solid #4c1d95",borderRadius:"0.85rem",padding:"0.65rem 0.7rem"}}>
          <div style={{fontSize:"0.75rem",fontWeight:800,color:"#c4b5fd",marginBottom:"0.35rem"}}>Vestige Spells</div>
          <div style={{display:"flex",gap:"0.3rem",flexWrap:"wrap"}}>{["Life Domain","Light Domain","Trickery Domain","War Domain"].map(d=>{const sel=vestigeDomain===d;return <button key={d} onClick={()=>setVestigeDomain(d)} style={{padding:"0.25rem 0.6rem",borderRadius:"0.55rem",cursor:"pointer",fontSize:"0.72rem",background:sel?"#7c3aed":"transparent",color:sel?"#fff":"#e2e8f0",border:"1px solid "+(sel?"#a78bfa":"#555")}}>{d}</button>;})}</div>
        </div>}
        {isBeastMaster&&<div style={{marginTop:"0.6rem",background:"#1a1035",border:"1px solid #4c1d95",borderRadius:"0.85rem",padding:"0.65rem 0.7rem"}}>
          <div style={{fontSize:"0.75rem",fontWeight:800,color:"#c4b5fd",marginBottom:"0.35rem"}}>{t("Primal Companion")}</div>
          <div style={{display:"flex",flexDirection:"column",gap:"0.3rem"}}>
            {Object.entries(BEAST_TYPE_OPTIONS).map(([name,info])=>{const sel=selBeastType===name;return(
              <div key={name} onClick={()=>setSelBeastType(name)} style={{display:"flex",alignItems:"flex-start",gap:"0.5rem",padding:"0.35rem 0.55rem",borderRadius:"0.55rem",cursor:"pointer",background:sel?"#4c1d9544":"transparent",border:"1px solid "+(sel?"#a78bfa":"#332255")}}>
                <span style={{flexShrink:0,width:"1rem",height:"1rem",borderRadius:"50%",border:"1px solid "+(sel?"#a78bfa":"#555"),background:sel?"#7c3aed":"transparent",marginTop:"1px"}}/>
                <div style={{flex:1}}><div style={{fontSize:"0.75rem",fontWeight:700,color:sel?"#e9d5ff":"#e2e8f0"}}>{name}</div><div style={{fontSize:"0.68rem",color:G.muted,lineHeight:1.3}}>{info[CURRENT_LANG==="da"?1:0]}</div></div>
              </div>
            );})}
      ...
      <div style={{background:"rgba(167,139,250,0.08)",border:"1px solid #a78bfa55",borderRadius:"0.85rem",padding:"0.75rem"}}>
        ...
        <GFld label={t("Species")}><select value={sp} onChange={e=>{setSp(e.target.value);speciesLockedRef.current=true;setSpeciesLocked(true);}} style={inp}>{Object.keys(SPECIES).map(s=><option key={s}>{s}</option>)}</select>{speciesData&&<div style={{marginTop:"0.4rem",background:G.card,borderRadius:"0.65rem",padding:"0.5rem 0.65rem"}}><div style={{fontSize:"0.65rem",color:"#a78bfa",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:"0.3rem",fontWeight:700}}>{t("Species Traits")}</div>{speciesData.traits.map((tr,i)=>{const dd=CURRENT_LANG==="da";const label=dd?(TRAIT_DA[tr]||tr):tr;const desc=TRAIT_DESC[tr]?.[dd?1:0];return <div key={i} style={{fontSize:"0.73rem",color:G.muted,marginBottom:"0.25rem"}}>- <b style={{color:"#cbd5e1"}}>{label}</b>{desc?<span style={{color:G.dim}}> — {desc}</span>:""}{TRAIT_PG[tr]?<span style={{color:G.dimmer}}> (PHB p.{TRAIT_PG[tr]})</span>:""}</div>;})}</div>}
          {sp==="Dragonborn"&&<div style={{marginTop:"0.4rem",background:G.card,borderRadius:"0.65rem",padding:"0.5rem 0.65rem"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"0.3rem"}}>{Object.keys(DRACONIC_ANCESTRY).map(c=>{const sel=dragonColor===c;return <button key={c} onClick={()=>setDragonColor(c)} style={{padding:"0.2rem 0.5rem",borderRadius:"0.45rem",fontSize:"0.72rem",border:"1px solid "+(sel?G.gold:"#334155"),cursor:"pointer",background:sel?G.gold:"transparent",color:sel?"#020817":"#f1f5f9",fontWeight:sel?700:400}}>{c} ({trDamageType(DRACONIC_ANCESTRY[c])})</button>;})}</div>
            <div style={{fontSize:"0.68rem",color:G.muted,marginTop:"0.35rem"}}>{t("Breath Weapon")}: {breathWeaponDice(level)} {trDamageType(DRACONIC_ANCESTRY[dragonColor])}</div>
          </div>}
  ...
  const statsPanel=(
    <div>
      ...
      <div className="mob-3col" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"0.5rem"}}>
        {AB.map(a=>(<div key={a} style={{background:a===primaryAb?"#2d2400":G.card,borderRadius:"0.75rem",padding:"0.6rem",border:"1px solid "+(a===primaryAb?G.gold:G.border),textAlign:"center",position:"relative"}}>{a===primaryAb&&<div style={{position:"absolute",top:"-0.55rem",left:"50%",transform:"translateX(-50%)",background:G.gold,color:G.bg,fontSize:"0.5rem",fontWeight:800,padding:"0.05rem 0.35rem",borderRadius:"0.3rem",textTransform:"uppercase",letterSpacing:"0.05em",whiteSpace:"nowrap"}}>{t("Key ability")}</div>}<div style={{fontSize:"0.65rem",color:a===primaryAb?G.gold:G.dim,letterSpacing:"0.1em",fontWeight:a===primaryAb?800:400}}>{a}</div><div style={{fontSize:"1.6rem",fontWeight:800,color:G.gold,marginTop:"0.2rem",lineHeight:1}}>{fin[a]}<span style={{fontSize:"0.95rem",fontWeight:700,marginLeft:"0.2rem"}}>({sgn(mf(fin[a]))})</span></div><div style={{fontSize:"0.55rem",color:G.dimmer,marginTop:"0.35rem"}}>{t("Base roll")}</div><input type="number" min={smode==="Manual"?8:3} max={smode==="Manual"?15:smode==="Rolled"?18:20} value={base[a]??8} disabled={smode==="Standard Array"} onFocus={e=>e.target.select()} onChange={e=>{
                // Don't clamp mid-typing — on mobile, digits arrive one keystroke at a time, and
                // clamping eagerly (e.g. "1" -> min 8) stomps the field before the second digit
                // lands, so a typed "14" could never survive. Only normalize on blur instead.
                const raw=e.target.value;const v=raw===""?"":Number(raw);
                if(smode==="Rolled")setRstats(prev=>({...prev,[a]:v}));
                else setMstats(prev=>{const next={...prev,[a]:v===""?0:v};if(pointBuySpent(next)>PB_BUDGET)return prev;return next;});
              }} onBlur={e=>{let v=Number(e.target.value)||(smode==="Rolled"?3:8);v=smode==="Rolled"?Math.max(3,Math.min(18,v)):Math.max(8,Math.min(15,v));if(smode==="Rolled")setRstats(prev=>({...prev,[a]:v}));else setMstats(prev=>({...prev,[a]:v}));}} style={{...inp,textAlign:"center",padding:"0.15rem",marginTop:"0.1rem",fontSize:"0.8rem",fontWeight:600,width:"3.2rem"}}/><div title={abilDesc(a)} style={{fontSize:"0.55rem",color:G.dim,marginTop:"0.3rem",lineHeight:1.15}}>{abilTag(a)}</div></div>))}
      </div>
